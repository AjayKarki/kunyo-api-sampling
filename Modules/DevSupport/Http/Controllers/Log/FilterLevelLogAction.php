...
/**
 * Class FilterLevelLogAction
 * @package Modules\DevSupport\Http\Controllers\Log
 */
final class FilterLevelLogAction
{
    ...
    public function __invoke(Request $request, $date, $level)
    {
        if ($level === 'all') {
        ...
        $entries = $this->logViewer->entries($date, $level)->paginate($this->getLimit());
