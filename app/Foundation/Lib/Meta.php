...
final class Meta
{
    ...
    {
        return \Cache::remember(static::resolveCacheKey($key), Cache::TIME_INTERVAL, function () use ($key, $default) {
            return app('db')
                ->table('settings')
                ->where('key', $key)
                ->value('value') ?? $default;
    ...
    {
        return \Cache::remember(static::resolveCacheKey($key), Cache::TIME_INTERVAL, function () use ($key) {
