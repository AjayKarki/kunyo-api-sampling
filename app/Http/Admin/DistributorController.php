...
class DistributorController extends BaseController
{
    ...
    {
        if ($request->ajax()) {
            return UserDatatable::get($this->userService->filterByRole($request->only('search.value', 'filter'), Role::$current[Role::ROLE_DISTRIBUTOR]), 'distributor');
        }
        $data['roles'] = $this->roleService->getRoles();
        $data['status'] = $this->userService->getCountByStatus(Role::$current[Role::ROLE_DISTRIBUTOR]);
    ...
    public function create()
    {
        $data = [];
        $data['roles'] = $this->roleService->getRoles();
        return view('admin.distributor.create', compact('data'));
    }
    ...
    */
    public function store(StoreRequest $request)
    {
        ...
        if ($user) {
            $user->roles()->sync((array) $this->roleService->getId(Role::$current[Role::ROLE_DISTRIBUTOR]));
        }
