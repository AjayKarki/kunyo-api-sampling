...
class SalesController extends BaseController
{
    ...
    {
        $role = Role::$current[Role::ROLE_SALES];
        if ($request->ajax()) {
            return UserDatatable::get($this->userService->filterByRole($request->only('search.value', 'filter'), $role), $role);
        }
    ...
    public function store(StoreRequest $request)
    {
        ...
        if ($user) {
            $user->roles()->sync((array) $this->roleService->getId(Role::$current[Role::ROLE_SALES]));
        }
